...
def sample_files_for_analysis(
    ...
    - Within each datasource: sort files by (extension_priority, filename) for deterministic selection
    - Pick first max_per_datasource files per datasource
    - Then take up to max_files across all datasources in deterministic order
    ...
    Returns:
        List of sampled RawFile records (deterministic order)
    """
    ...
    # Extension priority for deterministic sorting (higher = preferred)
    def extension_priority(ext: str) -> int:
        priority_exts = {'.pdf': 5, '.txt': 4, '.html': 3, '.htm': 3, '.docx': 2, '.doc': 2}
        return priority_exts.get(ext.lower(), 1)
    
    # Sort datasources deterministically (by UUID string representation or None first)
    datasource_ids = sorted(
        files_by_datasource.keys(),
        key=lambda ds_id: (ds_id is None, str(ds_id) if ds_id else "")
    )
    ...
    for ds_id in datasource_ids:
        ...
        # Deterministic sort: by (extension_priority, filename)
        # This ensures same files always selected in same order
        files_in_datasource_sorted = sorted(
            files_in_datasource,
            key=lambda record: (
                -extension_priority(Path(record.filename).suffix),  # Negative for descending
                record.filename.lower()  # Then by filename for tie-breaking
            )
        )
        
        # Deterministically select up to max_per_datasource files from this datasource
...
def auto_detect_playbook_and_chunking(
    ...
    # Sample files for analysis (deterministic)
    ...
    # Store sampled file list in evidence for explainability
    sampled_file_list = [
        {
            "filename": f.filename,
            "file_stem": f.file_stem,
            "data_source_id": str(f.data_source_id) if f.data_source_id else None,
    ...
    # Lower default threshold from 0.7 to 0.6
    ...
    # Store per-file analyses for explainability
    per_file_analyses = []
        ...
        try:
            ...
            if needs_chunking_detection:
                try:
                    ...
                    # Store per-file analysis with candidates
                    per_file_analysis = {
                        "filename": raw_file.filename,
                        "type": chunking_config.content_type.value,
                        "score": chunking_config.confidence,
                        "candidates": chunking_config.evidence.get("top_candidates", []),
                        "evidence": {
                            "all_scores": chunking_config.evidence.get("all_scores", {}),
                            "hint_applied": chunking_config.evidence.get("hint_applied", False),
                            "hint_type": chunking_config.evidence.get("hint_type"),
                            "confidence_gap": chunking_config.evidence.get("confidence_gap", 0.0),
                            "matched_patterns": chunking_config.evidence.get("matched_patterns", [])[:10],  # Limit for storage
                        }
                    }
                    per_file_analyses.append(per_file_analysis)
    ...
    if needs_chunking_detection and chunking_analyses:
        ...
        # Get confidence gap from evidence
        confidence_gap = best_analysis.evidence.get("confidence_gap", 0.0) if best_analysis.evidence else 0.0
        
        # Implement confidence gap rule: confidence_met = (best_score >= threshold) OR (best_score >= 0.55 AND confidence_gap >= 0.10)
        confidence_met = (
            best_analysis.confidence >= confidence_threshold
        ) or (
            best_analysis.confidence >= 0.55 and confidence_gap >= 0.10
        )
        
        # Get top candidates from best analysis
        top_candidates = best_analysis.evidence.get("top_candidates", []) if best_analysis.evidence else []
        
        logger.info(
            f"Auto-detected chunking config: content_type={best_analysis.content_type.value}, "
            f"confidence={best_analysis.confidence:.2f}, "
            f"confidence_gap={confidence_gap:.3f}, "
        ...
        # Build aggregated decision evidence
        aggregated_decision = {
            "type": best_analysis.content_type.value,
            "confidence": best_analysis.confidence,
            "confidence_gap": confidence_gap,
            "confidence_met": confidence_met,
            "top_candidates": top_candidates[:3],  # Top 3 candidates
        }
        
        # Add per-file analyses, aggregated decision, and sampled files to evidence
        enhanced_evidence = best_analysis.evidence.copy() if best_analysis.evidence else {}
        enhanced_evidence["per_file_analysis"] = per_file_analyses
        enhanced_evidence["aggregated_decision"] = aggregated_decision
        enhanced_evidence["sampled_files"] = sampled_file_list
        ...
        resolved_settings = {
            ...
            "all_scores": best_analysis.evidence.get("all_scores", {}) if best_analysis.evidence else {},
            "final_type": best_analysis.content_type.value,
            "final_confidence": best_analysis.confidence,
        }
        ...
        logger.info(f"Updated chunking_config with resolved_settings: content_type={resolved_settings['content_type']}, confidence={resolved_settings['confidence']:.2f}, confidence_met={confidence_met}")
        if not confidence_met:
            logger.info(
                "Auto-detected chunking confidence %.2f does not meet threshold (%.2f) or gap rule; "
...
def task_preprocess(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # But check if it was auto-detected - if so, don't use it (let it be auto-detected again)
        if product.playbook_id and not playbook_id:
            # Check if playbook was auto-detected in previous run
            if product.playbook_selection and product.playbook_selection.get("method") == "auto_detected":
                # Don't set playbook_id - let it be auto-detected again
                logger.info(f"Playbook was auto-detected previously, will auto-detect again")
            else:
                playbook_id = product.playbook_id
                logger.info(f"Using manually selected playbook from product: {playbook_id}")
        ...
        if product.playbook_id and not playbook_id:
            ...
            if product.playbook_selection and product.playbook_selection.get("method") == "auto_detected":
                ...
                playbook_id = product.playbook_id
                logger.info(f"Using manually selected playbook from product (after auto-detection): {playbook_id}")
        ...
        # Structured log: chunking_config passed to preprocess (must use resolved_settings when mode=auto + force)
        rs = (chunking_config or {}).get("resolved_settings") if isinstance(chunking_config, dict) else {}
        logger.info(
            "[CHUNKING_CONFIG] preprocess task passing to stage: chunk_size=%s chunk_overlap=%s "
            "chunking_strategy=%s content_type=%s from_resolved_settings=%s",
            rs.get("chunk_size") if isinstance(rs, dict) else None,
            rs.get("chunk_overlap") if isinstance(rs, dict) else None,
            rs.get("chunking_strategy") if isinstance(rs, dict) else None,
            rs.get("content_type") if isinstance(rs, dict) else None,
            bool(rs and isinstance(rs, dict) and rs.get("chunk_size") is not None),
        )
        ...
        if tracker:
            ...
            if product and result.status == StageStatus.SUCCEEDED:
                ...
                # Persist extraction_type from preprocess (digital_pdf | scanned_pdf | mixed; not content_type)
                extraction_type = result.metrics.get("extraction_type")
                if extraction_type:
                    product.extraction_type = extraction_type
                    logger.info(f"Updated product {product_id} with extraction_type: {extraction_type}")
                ...
                if playbook_selection:
                    ...
                    # Preserve the method (auto_detected vs manual) from preprocessing stage
                    if "method" not in playbook_selection or playbook_selection.get("method") is None:
                        # If method is missing, infer from whether playbook_id was originally None
                        if initial_playbook_id is None:
                            playbook_selection["method"] = "auto_detected"
                            playbook_selection["reason"] = playbook_selection.get("reason", "auto_detected")
                        else:
                            playbook_selection["method"] = "manual"
                ...
                elif final_playbook_id:
                    # If playbook was provided but no metadata exists, determine method
                    if initial_playbook_id is None:
                        # Auto-detected during pipeline
                        product.playbook_selection = {
                            "playbook_id": final_playbook_id,
                            "method": "auto_detected",
                            "reason": "auto_detected_during_pipeline",
                            "detected_at": datetime.utcnow().isoformat() + "Z",
                        }
                    else:
                        # Manually provided
                    ...
                    logger.info(f"Created playbook_selection for product {product_id}: {product.playbook_selection}")
...
def task_scoring(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Save chunk_metrics to database after scoring succeeds
        if product and result.status == StageStatus.SUCCEEDED:
            try:
                # Load metrics from storage (they were saved by scoring stage)
                metrics = storage.get_metrics_json()
                if metrics:
                    from primedata.services.s3_json_storage import save_product_json_field

                    s3_path, should_save_to_s3 = save_product_json_field(
                        product.workspace_id, product.id, "chunk_metrics", metrics
                    )
                    if should_save_to_s3 and s3_path:
                        product.chunk_metrics_path = s3_path
                        product.chunk_metrics = None  # Clear DB field
                    else:
                        product.chunk_metrics = metrics
                        product.chunk_metrics_path = None  # Clear S3 path if exists
                    db.commit()
                    logger.info(f"Saved {len(metrics)} chunk metrics to database for product {product_id}")
                else:
                    logger.warning(f"No metrics found in storage after scoring for product {product_id}")
            except Exception as e:
                logger.error(f"Failed to save chunk_metrics to database: {e}", exc_info=True)
                # Don't fail the task if saving metrics fails
    ...
    try:
        ...
        if product and result.status == StageStatus.SUCCEEDED:
            ...
            if fingerprint:
                ...
                # Calculate trust_score from database chunk_metrics instead of from fingerprint
                ...
                if chunk_metrics and isinstance(chunk_metrics, list) and len(chunk_metrics) > 0:
                    # Calculate trust_score from database chunk_metrics
                    from primedata.services.fingerprint import generate_fingerprint
                    from primedata.services.trust_scoring import aggregate_metrics_with_ai_ready
                    ...
                    # Generate fingerprint from database metrics
                    db_fingerprint = generate_fingerprint(chunk_metrics, preprocessing_stats)
                    
                    # Extract ai_trust_score from database-generated fingerprint
                    trust_score = db_fingerprint.get("ai_trust_score") or db_fingerprint.get("AI_Trust_Score")
                    if trust_score is not None:
                        product.trust_score = float(trust_score)
                        logger.info(f"Calculated trust_score={product.trust_score} from database chunk_metrics ({len(chunk_metrics)} chunks)")
                    else:
                        logger.warning(f"Could not extract trust_score from database-generated fingerprint for product {product_id}")
                else:
                    # Fallback: use fingerprint trust_score if chunk_metrics not available
                    trust_score = fingerprint.get("ai_trust_score") or fingerprint.get("AI_Trust_Score")
                    if trust_score is not None:
                        product.trust_score = float(trust_score)
                        logger.info(f"Using trust_score from fingerprint (chunk_metrics not available): {product.trust_score}")
                    else:
                        # Final fallback: try to get trust_score from metrics_result directly
                        trust_score = result.metrics.get("trust_score")
                        if trust_score is not None:
                            product.trust_score = float(trust_score)
                            logger.info(f"Using trust_score from result.metrics: {product.trust_score}")
                        else:
                            logger.warning(f"Could not determine trust_score for product {product_id}")
...
def task_indexing(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Structured log: chunking_config passed to indexing (must use resolved_settings when mode=auto + force)
        rs = (chunking_config or {}).get("resolved_settings") if isinstance(chunking_config, dict) else {}
        logger.info(
            "[CHUNKING_CONFIG] indexing task passing to stage: chunk_size=%s chunk_overlap=%s "
            "chunking_strategy=%s content_type=%s from_resolved_settings=%s",
            rs.get("chunk_size") if isinstance(rs, dict) else None,
            rs.get("chunk_overlap") if isinstance(rs, dict) else None,
            rs.get("chunking_strategy") if isinstance(rs, dict) else None,
...
def task_finalize(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        if product:
            ...
            # Only update current_version if this version is newer (prevents race conditions)
            if version > (product.current_version or 0):
                product.current_version = version
                logger.info(f"Product {product_id} current_version updated to {version}")
            else:
                logger.info(
                    f"Product {product_id} version {version} completed, but current_version remains {product.current_version} "
                    f"(version {version} is not newer)"
                )
            db.commit()
